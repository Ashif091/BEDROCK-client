...
const DocumentList: React.FC<DocumentListProps> = ({
  ...
  return (
    <div className="w-full h-60 overflow-hidden overflow-y-auto pb-3 border-y border-gray-200/30">
      ...
      <ul className="space-y-1">
        {documents.length > 0 ? (
          documents.map((doc) => (
            <li
              key={doc._id}
              className={`w-full  h-7 items-center text-center rounded-md flex gap-2 cursor-pointer ${
